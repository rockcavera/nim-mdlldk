...
# communication between the dll and mIRC must be by non-unicode (cstring).
...
newProcToExportA(debug):
  ...
  add(outData, " | echo -a data: " & $(cast[uint](addr data[0])))
  add(outData, " | echo -a parms: " & $(cast[uint](addr parms[0])))
  add(outData, " | echo -a -")
